...
### 2.6a Sky shell (the `/sky/` section only)

The Sky section is an app rather than a page — its visitors are mostly on a phone, at night, and want the tool, not the masthead — so its pages carry neither the header logos nor the navbar above. [sky/js/sky-nav.js](sky/js/sky-nav.js) draws one slim bar instead: the Aryabhat mark (the favicon, white halo per §1.6, linking home), the section's name in the brand font, the row of tools, and a menu listing the site's sections from the same list [js/layout.js](js/layout.js) uses. Where a page works from a place and a night, its night bar hangs directly under the shell. Styles are in [sky/css/sky.css](sky/css/sky.css) under SHELL. Do not add the logos or the navbar back to those pages.
